...
\usepackage{graphicx,url,paralist,todonotes,mathtools,float,setspace,rotating,booktabs,longtable,caption,subcaption,pdflscape,threeparttable,enumerate,xpatch}
...
\newcommand{\RJH}[1]{\todo[size=\tiny]{\fontsize{8}{9}\sf RJH: #1}}
\newcommand{\by}{\bm{y}}
\newcommand{\be}{\bm{e}}
\newcommand{\ba}{\bm{a}}
\newcommand{\bS}{\bm{S}}
...
\title{A note on non-negative reconciled forecasts for hierarchical and grouped time series}
...
\addresses{\textbf{Shanika L Wickramasuriya}\\
  ...
  Email: shanika.wickramasuriya@gmail.com\\[1cm]
...
Forecast reconciliation is the problem of ensuring that disaggregated forecasts add up to the corresponding forecasts of the aggregated time series. This is a common problem in manufacturing, for example, where time series of sales are disaggregated in several ways --- by region, product-type, and so on. There are often tens of thousands of forecasts at the most disaggregated level, and these are required to sum to give forecasts at higher levels of aggregation.

A simple solution would be to forecast the most disaggregated series and sum the results. However, this tends to give poor aggregate forecasts due to the low signal-to-noise ratio in the disaggregated time series. Instead, a better solution is to forecast all the series at all levels of aggregation, and then reconcile them so that the forecasts of the disaggregated series add up to the forecasts of the aggregated series. Least squares reconciliation was proposed by \citet{Hyndman2011}, whereby the reconciled forecasts are as close as possible (in the $L_2$ sense) to the original forecasts subject to the aggregation constraint. This result was extended by \citet{Hyndman2016} to a larger class of reconciliation problems, and by \citet{Wick2018} who showed that the resulting reconciled forecasts are minimum variance unbiased estimators.

The hierarchical and grouped time series that we encounter in practice can take either only non-negative values, such as revenue, demand and the number of visitor nights, or a mixture of positives and negatives, such as an organization's profits. If the time series are non-negative or consist of values on the real line, then the forecast reconciliation approaches discussed in \citet{Wick2018} can be applied. However, the top-down approach cannot be applied to time series that involve both positives and negatives, as it needs an appropriate set of proportions for distributing the aggregated forecast to its lower levels. 

Most of the applications that we have found are inherently non-negative in nature. In such circumstances, it is important to ensure that the reconciled forecasts do satisfy non-negativity, as forecasters and practitioners need to be able to make meaningful managerial decisions. Unfortunately, the MinT approach proposed by \citet{Wick2018} and its variants fail to guarantee this property even when the base forecasts are non-negative. This can be overcome by explicitly imposing the non-negativity constraints on the estimation procedure. One simple technique is to overwrite any negatives in the reconciled forecasts with zeros. However, the resulting approximate solution has ill-defined mathematical properties. This type of overwriting approximation method is used commonly in alternating least squares estimations. The problem is that they do not necessarily lower the objective function in each iteration, and therefore the convergence of the solution to the least squares minimum is not guaranteed. There are algorithms proposed in the literature that solve these problems in a mathematically rigorous way, and therefore this paper addresses the way in which the reconciled forecasts can be modified effectively by adding an additional non-negativity constraint. 
...
\section{Notation and MinT reconciliation}

For notation, we follow \citet{Wick2018} and let $\bm{y}_t$ denote a vector of observations at time $t$ comprising all series of interest including both disaggregated and aggregated time series. We also define $\bm{b}_t$ to be the vector of the most disaggregated series at time~$t$. These two vectors are connected via $\bm{y}_t=\bm{S}\bm{b}_t$ where $\bm{S}$ is the ``summing'' matrix showing how the various aggregated time series in $\bm{y}_t$ are constructed from the disaggregated series in $\bm{b}_t$.

Let $\hat{\bm{y}}_T(h)$ is a vector of initial (also referred as base or unreconciled) $h$-step-ahead forecasts, made at time~$T$, stacked in the same order as $\bm{y}_t$. The least squares reconciled forecasts are given by
\begin{align*}
\tilde{\bm{y}}_{T}(h) = \bm{S}\tilde{\bm{b}}_{T}(h)
\end{align*}
for some weighting matrix~$\bm{\Lambda}$ where $\tilde{\bm{b}}_{T}(h) = \left[\left(\bm{S}'\bm{\Lambda}_{h}^{-1}\bm{S}\right)^{-1}\bm{S}'\bm{\Lambda}_{h}^{-1}\right]\hat{\bm{y}}_{T}(h) = \left[\bm{J} - \bm{J}\bm{\Lambda}_{h}\bm{U}\left(\bm{U}'\bm{\Lambda}_{h}\bm{U}\right)^{-1}\bm{U}'\right]\hat{\bm{y}}_{T}(h)$. \citet{Wick2018} showed that setting
$\bm{\Lambda} =\var[\bm{y}_{t+h} - \hat{\bm{y}}_{t}(h)|\bm{\mathcal{I}}_{t}]\}$ to be the covariance matrix of the $h$-step-ahead base forecast errors minimizes the trace of 
$\var[\bm{y}_{t+h} - \tilde{\bm{y}}_{t}(h)|\bm{\mathcal{I}}_{t}]$ amongst all possible unbiased reconciliations.

\section{A quadratic programming solution}
To ensure that all entries in $\tilde{\bm{y}}_{T}(h)$ are non-negative, it is sufficient to guarantee that all entries in $\tilde{\bm{b}}_{T}(h)$ are non-negative. Even though the solution of $\tilde{\bm{b}}_{T}(h)$ is derived based on a minimization of the variances of the reconciled forecast errors across the entire structure, it is also apparent from the first expression that $\tilde{\bm{b}}_{T}(h)$ is the generalized least squares solution to the following regression problem:
\begin{align*}
...
\end{align*}
where $\breve{\bm{b}}_{T}(h)$ is the solution to the quadratic programming problem in Eq.\ \eqref{eq:quadprog}. This estimation problem is also referred to in the literature as non-negative least squares (NNLS).
...
It is easy to show that $\bm{S}'\bm{\Lambda}_{h}^{-1}\bm{S}$ is a positive definite matrix by using the fact that the matrix $\bm{S}$ is of full column rank and assuming that $\bm{\Lambda}_{h}$ is positive definite. Furthermore, these very simple non-negativity constraints will ensure that the feasible region is non-empty. Therefore, the quadratic programming problem in Eq.\ \eqref{eq:quadprog} has a unique global solution; i.e., there are no local minima apart from the global minimum \citep{Turl2015}.

In addition, \citet{Wick2018} strictly imposed the constraints of unbiasedness of the reconciled forecasts when deriving the solution of $\tilde{\bm{b}}_{T}(h)$. However, a slight bias may be introduced into the solution of the minimization problem in Eq.\ \eqref{eq:quadprog} because we are not considering that property here. 

The well-known quadratic programming problem in Eq.\ \eqref{eq:quadprog} is easy to solve for small scale hierarchical or grouped structures using the \texttt{quadprog} package for R, which is designed to handle dense matrices \citep{quadprog2013}. However, use of this package becomes problematic as the size of the structure increases, as it requires matrices to be stored in a sparse format to save memory and increase the computational efficiency. Furthermore, assuming a dense matrix structure for $\bm{\Lambda}_{h}$ can also be an issue.

\subsection{First-order optimality conditions}
This section discusses first-order necessary conditions for $\breve{\bm{b}}_{T}(h)$ to minimize the non-negative least squares problem. 
...
The feasible region that corresponds to a given set of consistent inequality constraints is given by a volume in multidimensional space. Any point in this region (a feasible solution) consists of one of two types of variables, depending on whether it lies on the boundary or within the feasible region. The former set of variables is referred to as the active set. These are variables that can only lower the objective function by moving away from the feasible region, and must be assumed to lie on the boundary in order to ensure the feasibility of the solution. The latter set of variables is referred to as the passive set. The active set method relies on the idea that, if the true active set is known in advance, the solution to the least squares problem is given by the unconstrained least squares solution, obtained by using the variables in the passive set and setting the regression coefficients for the active set variables to zero. In laymen's terms, this can be interpreted as an attempt to forecast hierarchical and grouped time series by simply refining the structure through the removal of the bottom-level series that are responsible for any negative or zero reconciled forecasts. In practice, though, it is difficult to define such sets in advance. Therefore, we have to search for the optimal set iteratively. 

The preliminary algorithm starts with a feasible vector; specifically, a vector of zeros, meaning that the passive set is empty. The algorithm consists of two nested loops. The outer loop determines an index in the active set that is to be moved to the passive set. Then, the unconstrained least squares solution is obtained by using the variables in the passive set. If the solution is infeasible, the variables must be modified to maintain the feasibility, which is done using the inner loop. Within the loop, a convex combination of the old and new solutions is taken. The old solution is feasible, but has a worse fit than the new infeasible solution. Therefore, a weight parameter is chosen so as to minimize the fit while retaining as many variables as possible in the passive set. The variables that fall on the boundary are moved from the passive set to the active set. After updating the sets, an unconstrained least squares solution for the current passive set is obtained. This loop executes until no violated variables are found in the passive set. Thus, it is clear that the active set method is propagated by finding the solution of the passive set variables as an equality-constrained least squares problem, where the equalities are defined using all of the variables in the active set. The optimality of the solution is tested by checking whether the KKT conditions are satisfied. If they are, the outer loop is terminated. The algorithm will converge in a finite number of iterations, as each step is designed in such a way that the objective function is strictly decreasing with each iteration and there are finite possible combinations of the passive set.  

In the presence of multiple right hand side columns in the normal equations (in other words, multiple columns of $\hat{\bm{y}}_{T}(h)$ representing the base forecasts of different forecast horizons), the passive set that corresponds to each column may not be unique. One way to address this problem is to solve each column independently, which is sometimes referred to as ``column serial''. Even though this procedure is straightforward, it may be computationally inefficient because of performing redundant computations. \citet{Bro1997} further developed the standard active set method for handling multiple right hand side variables, which is referred to as fast non-negative least squares (FNNLS). This algorithm precomputes certain cross-product matrices that are used in the normal equation formulation of the least squares solution. This tends to improve the performance, especially when $\bm{S}$ has many more rows than columns. Later on, \citet{VanBenthem2004} proposed a variation of this algorithm called fast combinatorial non-negative least squares in order to speed up the computations further, by rearranging the multiple columns of $\hat{\bm{y}}_{T}(h)$ with similar passive sets in each iteration. This reduces the computational burden of finding pseudoinverses, as it allows the pseduoinverse to be computed once per group and applied to every member of that group. A dramatic increase in computational speed can be achieved in cases where the number of right hand side columns is much larger than the number of possible passive sets that correspond to a given situation.

Even though these algorithms guarantee to terminate within a finite number of iterations, they have very pessimistic upper bounds on the numbers of iterations needed to reach the optimal solution. Suppose that we have $n$ series at the bottom level, resulting in a total of $2^{n}$ possible active sets. Since the solution is unique, one of the $2^{n}$ combinations corresponds to the optimal active set. Therefore, finding the optimal set gives rise to a problem known as combinatorial difficulty. The main shortcoming of the standard active set method is that the algorithm is initialized by assuming an empty passive set, and only one variable is added in each outer loop. Although QR updating and down-dating techniques are used to speed up the computations, more iterations (in other words, more time) might be required to find the optimal active set when handling large scale non-negative least squares problems. Therefore, several alternatives have been proposed in the literature for overcoming these difficulties.

One possibility is to initialize the outer loop using a non-empty passive set. A simple way to introduce such a set is to solve the original unconstrained least squares problem and then overwrite any negative values with zeros. This type of initialization is used widely in the field of multivariate curve resolution \citep{Andrew1998, Gemperline2003}. Such a guess would be reasonable in cases where there are very few active constraints or when non-negativity is enforced in order to remove any observational or measurement errors that may be present \citep{Wang2012}. 

\citet{Wang2012} proposed an alternative initialization strategy for accelerating the performance of the active set method. This choice was inspired by one of the characteristics of gradient projection methods, namely the fact that the objective function decreases rapidly in the early stages of the algorithm, but slows down considerably after a few iterations. Thus, they have combined the good properties of projection-based methods with those of active set methods by using the projection method to initialize the active set method. They showed theoretically that the sequence of sets generated by the projection-based algorithm converges to the optimal passive set of the active set method. Furthermore, they introduced two measures for evaluating the quality of the initialization of the active set method, as this is one of the main driving factors that affects the performance. One such measure is called the ``recall''; it indicates how much benefit can be achieved from such an initialization. The second measure is the ``precision'', which indicates how much extra work has to be carried out to remove any unnecessary variables from the initial guess. They compared their choice of initialization with that of the overwrite solution of unconstrained least squares, and found the former to be better.

The second approach to enhancing the speed of the active set method involves including more than one variable in each outer loop. This should be handled very carefully, as it could lead to endless loops in the algorithm. Thus, these types of methods, which are referred to in the literature as block principal pivoting methods, include a procedure for selecting a group of variables to exchange, and a backup rule in order to ensure the finite termination of the algorithm. Of the different variations of the active set methods that we have discussed thus far, we are particularly interested in this type of active-set-like method, as they are quite efficient for handling large scale strictly convex quadratic programming problems with non-negativity constraints. The method is referred to as the active-set-like method because of its similarity to the active set method. 
...
If $\text{card}(\bar{I}_{1} \cup \bar{I}_{2}) > 1$, where $\text{card}(X)$ is the cardinality of set $X$, the algorithm is known as a block principal pivoting algorithm, whereas if $\text{card}(\bar{I}_{1} \cup \bar{I}_{2}) = 1$, it is known as a single principal pivoting algorithm. The active set method that we discussed earlier can be seen as a single principal pivoting algorithm. However, they are not identical. The first block principal pivoting algorithm for solving a strictly monotonic LCP is due to the work of \citet{Kost1978}. Unfortunately, unlike the active set method, in which the variable to be exchanged is chosen carefully with the aim of reducing the objective function, the use of blocks of variables for exchange can lead to a cycle, meaning that it is not guaranteed to provide the optimal solution. Although this occurs rarely, it is problematic. \citet{Judice1989} evaluated the performance of this algorithm on several test problems with a symmetric positive definite matrix. Their experiments concluded that this algorithm is quite efficient for handling large-scale strictly monotonic LCPs. Furthermore, they noted that the occurrence of cycles is extremely rare. In a later paper, \citet{Judice1994} proposed an extension of this algorithm to include finite termination, by incorporating Murty's single principal pivoting algorithm. Even though the algorithm proposed by \citet{Murty1974} has finite termination, convergence can be very slow for applications with large numbers of variables, as it changes only one variable per iteration. However, this algorithm is still beneficial for obtaining a complementary basic solution with a smaller number of infeasibilities than before. The steps of the hybrid algorithm are given below. 
...
\end{align*} 
where $\tilde{\bm{b}}$ is the original unconstrained least squares solution with positive diagonal elements for $\bm{\Lambda}_{h}$. This algorithm always terminates within a finite number of iterations, because ninf decreases with time over the course of the algorithm. This is a consequence of the use of Murty's method.
...
\subsubsection*{Step 2: Solving the subproblem}
...
\begin{algorithm}
	...
	\begin{spacing}{1.3}
		\begin{algorithmic}[1]
			...
			\State {Use $\bm{b}^{c}$ to find an approximate feasible solution $\ddot{\bm{b}}^{+}$ that satisfies $q(\ddot{\bm{b}}^{+}) \leq q(\bm{b}^{c})$.}
...
This section highlights the roles of the remaining tuning parameters and the optimized values that are obtained in the literature by testing several applications. 

\begin{itemize}
	\item $\eta$ and $\theta$ control the amount by which the objective function should be decreased and the number of backtracking reductions to be performed, respectively. In the literature, $\eta = 10^{-4}$, while $\theta = 0.4$ is used in order to get a sufficiently large step size with fewer reductions \citep{Bonettini2009}.
	\item $\alpha_{\text{min}}$ and $\alpha_{\text{max}}$ are the lower and upper bounds of the step-length parameter $\alpha_{k}$, in order to avoid unnecessary extreme values. Even though a very large range is defined for the BB-type rules in practice, \citet{Bertero2013} found the interval ($10^{-5}, 10^{5}$) to be suitable for the generalized BB rules.
...
It can be seen that the non-negative reconciled forecasts from the OLS approach showed slight gains over the negative reconciled forecasts at each forecast horizon considered (with very rare exceptions). These gains are most pronounced at the disaggregated levels. In addition, WLS$_{v}$ and MinT also showed gains at the most disaggregated level, though the consideration of aggregated levels sometimes introduced slight losses. As these gains or losses are negligible, it can be argued that the non-negativity constraint does not significantly affect the performances of the forecast reconciliation approaches. However, it is very useful in real applications for making meaningful managerial decisions and policy implementations. 
