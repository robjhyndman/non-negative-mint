...
\begin{algorithm}
	...
	\begin{spacing}{1.3}
		\begin{algorithmic}[1]
			\Require Choose a feasible initial solution $\ddot{\bm{b}}^{0}$. Set the parameters $\eta, \theta \in (0, 1)$, $0 < \alpha_{\tiny \textnormal{min}} < \alpha_{\tiny \textnormal{max}}$, and a positive integer $M$. Use a diagonal scaling matrix $\bm{D} = \textrm{diag}(d_1, d_2, \dots, d_n)$ with elements $d_i = 1 / (\bS'\bm{\Lambda}_h^{-1}\bS)_{ii}$ for $i = 1, 2, \dots, n$.
			\ForAll{k in 0, 1, 2, \dots}
			\State\multiline{Choose the step-length parameter $\alpha_{k} \in [\alpha_{\tiny \textnormal{min}}, \alpha_{\tiny \textnormal{max}}]$ using the alternation strategy proposed by \citet{Bonettini2009}.}
			\State{Projection: $\bm{z}^{k} = [\ddot{\bm{b}}^{k} - \alpha_{k}\bm{D}_{k}\ddot{\bm{g}}(\bm{b}^{k})]_{+}$, where $(x)_+ = \textrm{max}(0, x)$.}
...
\begin{table}[p]
	...
	\begin{threeparttable}
		\begin{tabular}{llrrrrrrrrrrr}
			...
			& BPV & \pmb{3.0} & \pmb{2.8} & \pmb{2.9} & \pmb{2.8} & \pmb{2.6} & & \pmb{2.3} & \pmb{2.3} & \pmb{2.2} & \pmb{2.1} & \pmb{2.0} \\ \\[-0.3cm]
			\cline{3-7} \cline{9-13} \\[-0.3cm]
			WLS$_{v}$ & ABB & \pmb{2.0} & \pmb{1.9} & \pmb{1.9} & \pmb{1.9} & \pmb{1.8} & & \pmb{2.1} & \pmb{2.0} & \pmb{2.0} & \pmb{1.9} & \pmb{1.8} \\
			...
			& BPV & \pmb{38.4} & \pmb{37.8} & \pmb{37.5} & \pmb{36.2} & \pmb{33.7} & & \pmb{38.2} & \pmb{37.7} & \pmb{ 37.3} & \pmb{36.0} & \pmb{33.6} \\
			\bottomrule
		\end{tabular}
		\begin{tablenotes}
